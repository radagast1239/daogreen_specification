...
for (const f of files) {
  ...
  const rowsWithImg = new Set();
  for (const img of images) {
    for (const m of parsed.materials) {
      if (m._sheet === img.sheetName && (m._row === img.row || m._row === img.row + 1 || m._row === img.row - 1)) {
        rowsWithImg.add(m.name);
      }
    }
  }
  console.log("Rows matched by position:", rowsWithImg.size);
  ...
  if (images.length && parsed.materials.length) {
    ...
    for (const img of sample) {
      ...
      console.log(`  img row=${img.row} col=${img.col} sheet=${img.sheetName} ->`, near.map((m) => `r${m._row}:${m.name.slice(0,40)}`).join("; "));
    }
  }
}
