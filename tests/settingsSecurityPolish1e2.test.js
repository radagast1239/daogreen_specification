...
describe("settings security polish 1e2", () => {
  it("detects primary via stable env-primary id from auth seed", () => {
    // Hardened auth: ENV key is not seeded into SQLite; env-primary rows are deleted.
    expect(authJs).toContain("env-primary");
    expect(authJs).toMatch(/DELETE FROM admin_users WHERE id = \?/);
    expect(authJs).toMatch(/Never insert ADMIN_KEY|ENV key must not live in SQLite/);
