/**
 * PHASE 1A-2C2D2 — apply-orchestration для atomic multi-wall delete
 * (wall.bulkDelete), mirroring applyWallDelete.js's contract for the bulk
 * case. Leaf helper: не импортирует React/DOM/HistoryModel/geometryCommands
 * напрямую — только вызывает переданный runGeometryCommand (production
 * dispatcher) ровно один раз с каноническим payload wall.bulkDelete. Не
 ...
 * }
 */
export function applyWallBulkDelete({ wallIds, runGeometryCommand }) {
  ...
  const result = runGeometryCommand({ type: "wall.bulkDelete", wallIds });
