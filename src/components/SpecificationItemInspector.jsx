    ...
    <aside className="spec-inspector" aria-label={`Подробнее: ${item.name}`} data-item-id={item.id}>
      ...
      <div className="spec-inspector__body">
        <section><h3>Основное</h3>
          ...
          <label>Наименование<input value={item.name || ""} onChange={(e) => patch({ name: e.target.value })} /></label>
        ...
        <section><h3>Закупка</h3>
          <label>Поставщик<input value={item.supplier || ""} onChange={(e) => patch({ supplier: e.target.value })} /></label>
