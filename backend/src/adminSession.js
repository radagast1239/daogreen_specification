...
function signPayload(payloadB64) {
  ...
  return createHmac("sha256", secret).update(`v${SESSION_VERSION}.${payloadB64}`).digest();
}
...
export function createAdminSessionToken(nowMs = Date.now()) {
  ...
  if (!sig) return null;
  return `v${SESSION_VERSION}.${payloadB64}.${b64url(sig)}`;
}

export function verifyAdminSessionToken(token, nowMs = Date.now()) {
  ...
  if (ver !== `v${SESSION_VERSION}` || !payloadB64 || !sigB64) return null;
